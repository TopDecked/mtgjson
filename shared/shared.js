...
exports.getSetsToDo = function (startAt) {
    ...
    process.argv.slice(startAt).forEach(function (arg) {
        if (arg === "allsets") {
            setsToDo = C.SETS.map(function (SET) {
                return SET.code;
            });
    ...
    setsToDo = setsToDo.filter(function (set) {
        return !setsNotToDo.includes(set);
    });
...
exports.performSetCorrections = function (setCorrections, fullSet) {
    ...
    setCorrections.forEach(function (setCorrection) {
        if (setCorrection === "noBasicLandWatermarks") {
            ...
            cards.sort(cardNumberComparator).forEach(function (card) {
            ...
            cards.forEach(function (card) {
                exports.updateStandardForCard(card);
            });
        } else {
            var cardsToRemove = [];
            ...
            cards.forEach(function (card) {
                if (setCorrection.match && (setCorrection.match === "*" || (Object.keys(setCorrection.match).every(function (key) {
                    ...
                    if (setCorrection.remove)
                        setCorrection.remove.forEach(function (removeKey) {
                            delete card[removeKey];
                        });
                    ...
                    if (setCorrection.setLegality) {
                        Object.keys(setCorrection.setLegality).forEach(function (legalityType) {
                            ...
                            if (!foundExisting)
                                card.legalities.push({
                                    format: legalityType,
                                    legality: legalityValue
                                });
                        });
                    }
                    ...
                    if (setCorrection.deleteLegality && card.hasOwnProperty("legalities"))
                        card.legalities = card.legalities.filter(function (cardLegality) {
                            return !setCorrection.deleteLegality.includes(cardLegality.format);
                        });
                    ...
                    if (setCorrection.incrementNumber) {
                        if (cardsToIncrementNumber.includes(card.name)) {
                            var cardCount = cardsToIncrementNumber.filter(function (c) {
                                return c === card.name;
                            }).length;
            ...
            if (cardsToRemove.length > 0)
                cards = cards.filter(function (card) {
                    return !cardsToRemove.includes(card);
                });

            if (setCorrection.copyCard || setCorrection.importCard || setCorrection.addCard) {
                var newCard;
                if (setCorrection.copyCard)
                    newCard = clone(cards.find(function (card) {
                        return card.name === setCorrection.copyCard;
                    }));
                else if (setCorrection.importCard)
                    newCard = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "json", setCorrection.importCard.set + ".json"), {
                        encoding: "utf8"
                    })).cards.find(function (card) {
                        return card.name === setCorrection.importCard.name;
                    });
                ...
                if (setCorrection.replace)
                    Object.keys(setCorrection.replace).forEach(function (key) {
                        newCard[key] = setCorrection.replace[key];
                    });
                if (setCorrection.remove)
                    setCorrection.remove.forEach(function (removeKey) {
                        delete newCard[removeKey];
                    });
    ...
    cards.forEach(function (card) {
        ...
        if (["YYYY-MM-DD", "YYYY-MM", "YYYY"].some(function (dateFormat) {
                return !moment(card.releaseDate, dateFormat).isValid();
            })) {
    ...
    cards.forEach(function (card) {
        if (card.hasOwnProperty("legalities"))
            card.legalities = card.legalities.sort(function (a, b) {
                var al = a.format.toLowerCase().charAt(0);
                var bl = b.format.toLowerCase().charAt(0);
        ...
        if (card.hasOwnProperty("foreignNames")) {
            card.foreignNames = card.foreignNames.filter(function (a) {
                ...
                var al = a.language.toLowerCase().charAt(0);
                var bl = b.language.toLowerCase().charAt(0);
                return (al < bl ? -1 : (al > bl ? 1 : 0));
            });
        }
    });
    ...
    // Generate ID
    cards.forEach(function (card) {
        card.id = sha1(fullSet.code + card.name + card.imageName);
    });
    fullSet.cards = cards;
};
...
exports.saveSet = function (set, callback) {
    ...
    fs.writeFile(path.join(__dirname, "..", "json", fn), JSON.stringify(set, null, '  '), {
        encoding: "utf8"
    }, callback);
};
...
exports.processSet = function (setCode, processFunction, callback) {
    tiptoe(
        function getJSON() {
            fs.readFile(path.join(__dirname, "..", "json", setCode + ".json"), {
                encoding: "utf8"
            }, this);
        },
